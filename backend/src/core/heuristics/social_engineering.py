...
class SocialEngineeringHeuristic(BaseHeuristic):
    """Detects phishing language using a regex-based keyword threat feed across multiple categories."""
    ...
    def _load_threat_intel(self) -> None:
        """Loads phishing keyword categories from JSON and compiles a single combined regex."""
    ...
    async def evaluate(self, email_data: Dict[str, Any]) -> Dict[str, Any]:
        """Scores the email body for social engineering language patterns.

        Args:
            email_data: Parsed email payload dict.

        Returns:
            Dict with 'score' (int, capped at 40) and 'details' (dict of findings).
        """
